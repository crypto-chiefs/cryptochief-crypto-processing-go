...
func TestWalletGenerate_LabelWireShape(t *testing.T) {
	...
	if _, err := c.Wallets.Generate(context.Background(), &GenerateWalletRequest{
	...
	srv2 := captureServer(t, `{"type":"master","address":"0xbeef","chain_family":"EVM","frozen":false,"master_wallet_address":null,"callback_url":null}`, &path, &noLabelBody)
	c2, _ := New("m", "k", WithBaseURL(srv2.URL), WithRetries(0))
	if _, err := c2.Wallets.Generate(context.Background(), &GenerateWalletRequest{
...
// The wallet-info shape always carries master_wallet_address and callback_url,
// and sends null when there is none. A master wallet has neither; decoding must
// not fail, and both must read as unset.
func TestWallet_NullMasterAndCallbackDecode(t *testing.T) {
	var path, body string
	srv := captureServer(t, `{"type":"master","address":"0xbeef","chain_family":"EVM","frozen":false,"master_wallet_address":null,"callback_url":null}`, &path, &body)
	...
	if w.MasterWalletAddress != "" || w.CallbackURL != "" {
		t.Errorf("null must decode as unset, got master=%q callback=%q", w.MasterWalletAddress, w.CallbackURL)
	}
	if w.Frozen {
